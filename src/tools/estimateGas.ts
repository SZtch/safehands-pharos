...
export const estimateGasTool = {
  name: "estimate_gas",
  description:
    "Estimate gas cost for a swap or transfer before executing. Returns gas units, cost in PROS/USD, and whether the wallet has sufficient gas.",
  inputSchema: estimateGasSchema,
};

const QUOTE_WALLET = "0x0000000000000000000000000000000000000001";

export async function handleEstimateGas(raw: EstimateGasInput) {
  ...
  try {
    ...
    const gasCostPROS = formatEther(gasCostWei);
    ...
    try {
      ...
      if (priceQuote.routeAvailable) {
        gasCostUsd = (parseFloat(gasCostPROS) * parseFloat(priceQuote.amountOut)).toFixed(6);
    ...
    return ok({
      ...
      recommendedGas: { value: recommendGasWithBuffer(gasEstimate).toString(), unit: "gas", bufferPct: DEFAULT_GAS_BUFFER_PCT },
      gasPrice: { value: formatGwei(gasPrice), unit: "gwei" },
      gasCostPROS: { value: gasCostPROS, unit: "PROS" },
      gasCostUsd: gasCostUsd ? { value: gasCostUsd, unit: "USD" } : null,
      walletBalance: { value: formatEther(balance), unit: "PROS" },
      isSufficient,
      priceSourceStatus,
      chainId: CHAIN_ID,
