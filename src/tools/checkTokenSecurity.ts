...
export async function handleCheckTokenSecurity(raw: CheckTokenSecurityInput) {
  ...
  if (chainId === CHAIN_ID) {
    return fail(
      "CHAIN_NOT_SUPPORTED_BY_GOPLUS",
      `GoPlus Security API does not support Pharos Atlantic Testnet (chain ${CHAIN_ID}). ` +
        "Token security data is unavailable for this chain. Use token_registry_status to check " +
        "whether a token address is canonical, custom, or unknown on Pharos.",
      false,
