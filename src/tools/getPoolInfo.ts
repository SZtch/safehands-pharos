...
// Pharos Network. API key is optional: public mode is tried first
...
export const getPoolInfoSchema = z.object({
  tokenA: z.enum(["PROS", "USDC", "USDT"]).describe("First token in the pair"),
  tokenB: z.enum(["PROS", "USDC", "USDT"]).describe("Second token in the pair"),
});
...
export const getPoolInfoTool = {
  name: "get_pool_info",
  description:
    "Fetch DODO/FaroSwap route-derived liquidity info for a token pair on Pharos Mainnet. API key is optional; public mode is attempted first.",
  ...
  try {
    ...
    if (!routeJson.data || routeJson.status !== 200) {
      ...
      return ok({
        ...
        available: false,
        message: "No active DODO/FaroSwap route found for this pair on Pharos Mainnet.",
