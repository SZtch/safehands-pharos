...
/**
 * Resolves a token symbol (e.g. "PROS") or address to its on-chain address.
 */
export function resolveTokenAddress(token: string): `0x${string}` {
 ...
 */
export function resolveTokenDecimals(token: string): number {
  const upper = token.toUpperCase();
  const decimals = activeTokenDecimals();
  if (decimals[upper] !== undefined) return decimals[upper];
  const lower = token.toLowerCase();
  if (decimals[lower] !== undefined) return decimals[lower];
 ...
 */
export function fromWei(weiInput: string | number, decimals: number): string {
  const weiStr = String(weiInput);
  if (weiStr.includes(".")) return weiStr;
...
/**
 * Resolves the FaroSwap AUTO slippage tolerance based on token pair types.
 * @see https://docs.faroswap.xyz
 */
export function resolveAutoSlippage(fromToken: string, toToken: string): number {
  const from = fromToken.toUpperCase();
  const to = toToken.toUpperCase();

  const stablecoins = new Set(["USDT", "USDC", "DAI", "BUSD", "CIRCLE_USDC"]);
  if (stablecoins.has(from) && stablecoins.has(to)) {
    return 0.1; // 0.1% for stablecoin pairs
  }

  const majors = new Set(["BTC", "WBTC", "ETH", "WETH", "USDT", "USDC", "CIRCLE_USDC"]);
  if (majors.has(from) && majors.has(to)) {
    return 0.5; // 0.5% for major pairs
  }

  return DODO_DEFAULT_SLIPPAGE; // 3% default for all others
}
...
export async function _getDodoRouteCore(params: {
  ...
  const slippage = params.slippage ?? resolveAutoSlippage(params.fromToken, params.toToken);
  ...
  if (!response.ok) {
    if (response.status === 401 || response.status === 403) {
      throw new Error(
        `DODO_API_AUTH_REQUIRED: DODO route API rejected public mode${usedApiKey ? " or provided key" : "; configure DODO_API_KEY if this endpoint requires authentication"}`
      );
    }
  ...
  const json = (await response.json()) as DodoRouteResponse;
 ...
 * (like alternate USDC) if the canonical tokens have no liquidity on Atlantic Testnet.
 * Pacific Mainnet never falls back to non-Pacific token addresses.
 */
...
/**
 * Checks whether a token is the native PROS token.
 */
export function isNativeToken(token: string): boolean {
  const upper = token.toUpperCase();
  if (upper === "PROS") return true;
  return token.toLowerCase() === PROS_ADDRESS.toLowerCase();
}
