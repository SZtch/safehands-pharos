...
export function evaluateActionPolicy(input: ActionPolicyInput): ActionPolicyResult {
  ...
  if (input.actionType === "x402_pay_and_fetch") {
    ...
    if (input.paymentTokenAddress && input.paymentTokenAddress.toLowerCase() !== USDC_ADDRESS.toLowerCase()) {
      ...
      pushCheck(
        ...
        "x402_payment_token",
        status,
        `Payment token ${input.paymentTokenAddress} is not canonical Circle USDC ${USDC_ADDRESS}.`,
        reasons,
        requiredActions,
        status === "fail" ? "x402 payment token is not canonical USDC." : undefined,
        "Use docs-verified Pharos testnet USDC or label the token as project-configured."
      );
    }
  }
  ...
  if (checks.some((c) => c.name.includes("funding") && c.status === "fail")) decision = "REQUIRE_FUNDING";
  if (checks.some((c) => c.name.includes("token") && c.status === "unknown")) decision = "REQUIRE_TOKEN_REVIEW";
