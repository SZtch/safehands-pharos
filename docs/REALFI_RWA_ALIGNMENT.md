...
| Settlement discipline | x402/USDC payment rails with SSRF + redirect guards, token allowlist (Pacific USDC/WPROS), per-call and daily caps; paid `/paid/*` endpoints settle via a remote facilitator with a dedicated settlement key | **Live** — demo scenario 12; `GET /paid/risk-report` returns a real HTTP 402 on mainnet USDC |
