...
Everything in the "Live today" column below is deployed and verifiable right now: contracts on Pharos Pacific Mainnet (chain `1672`), a read-only HTTP API you can self-host as a reference backend, and a runnable demo. The zero-custody hosted agent is live on [Anvita Flow](https://flow.anvita.xyz/home) as `safehands` (Agent Carnival Phase 2). Nothing in that column is aspirational: every row is running code you can verify today (known limits are disclosed honestly in [SECURITY.md](../SECURITY.md) and [PHAROS_RPC.md](./PHAROS_RPC.md)).
...
| Risk memory other contracts can compose on | [`SafeHandsRegistry`](https://www.pharosscan.xyz/address/0x428e02bf85412e7242d991cd6725ec59e8b06c8d) Merkle risk roots + on-chain `verifyRiskRecord` view + keyless `verify_risk_inclusion` proof tool | **Live**: chain 1672; contract + verify path, and the first production risk batch is committed on-chain (`query` reports `hasCommittedRoot:true` and returns records) |
...
Or call it as an agent: the hosted SafeHands agent is live on [Anvita Flow](https://flow.anvita.xyz/home) as `safehands` (Agent Carnival Phase 2), discoverable and callable by any Steward Agent. To hit the HTTP API directly, self-host the read-only reference backend locally (`npm run build && node dist/api/server.js`, default port `4022`), then:
