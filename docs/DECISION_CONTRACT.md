...
**The TS score is advisory evidence, not a decider.** On write paths the score never gates execution directly: the tools feed the assessment into the policy engine (`riskEvidenceFromAssessment` → `ActionPolicyInput.risk`), where three checks make the actual decision: `risk_score` (fail when score > `RISK_BLOCK_THRESHOLD` → `BLOCK`, never confirmable), `risk_counterparty` (fail when the counterparty dimension ≥ `COUNTERPARTY_CRITICAL_THRESHOLD` → `BLOCK`), and `risk_degraded` (unknown when a critical signal was missing → `REQUIRE_CONFIRMATION`, confirmable). `evaluateActionPolicy` + `enforceWriteDecision` (with `requireRiskEvidence: true`, which fails closed if the evidence was not wired in) is the **single decision point**; the `recommendation` field remains for reports and the on-chain assessment record.
