...
Current as of v2.3.0 — Pharos Pacific Mainnet (chain `1672`), Anvita Flow hosted agent, live contracts.
...
The fully-hosted, zero-custody agent lives on [Anvita Flow](https://flow.anvita.xyz/home). Any Steward Agent on the marketplace can discover and call it — no server, no keys, no custody. To try the engine on your own machine without cloning:
...
```bash
npm ci && npm run build
npm run demo          # or: npx safehands-pharos --demo
```
