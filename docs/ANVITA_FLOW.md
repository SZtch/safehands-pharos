...
> **Hosted agent**: the SafeHands Agent is live on **Anvita Flow** ([flow.anvita.xyz/discover](https://flow.anvita.xyz/discover)) as `safehands` (Agent Carnival Phase 2); discoverable and callable by any Steward Agent; no install required. It runs the bundled zero-dependency engine ([`anvita/safehands/`](../anvita/safehands/)) in no-custody, read-only mode: it analyzes wallets, tokens, contracts, vaults, pools, approvals, transfer/swap and RealFi intents; resolves names to canonical addresses (`resolve_alias`); reads balances, portfolio value, and active approvals; recognizes contracts by codehash; detects permissioned-RWA tokens; and returns a deterministic allow/warn/block verdict with a ready-to-show plain-text `chatSummary`. It signs and broadcasts nothing. The full command reference is in [`anvita/safehands/references/`](../anvita/safehands/references/). Calls are billed by the Anvita Flow platform at **0.01 USDC per call** on its x402/USDC rail (Anvita Flow requires a per-call price at publish); this platform fee is separate from the optional self-hosted `/paid/*` bundle below. To exercise the raw HTTP endpoints referenced below (`/tools/*`, `/wallet/prepare`, `/broadcast/signed`, `/paid/*`), self-host the read-only reference backend locally (`npm run build && node dist/api/server.js`, default port 4022) and hit `http://localhost:4022`.
