...
**Permissioned RWA tokens (ERC-3643 / ERC-1400).** Pharos is an RWA-first chain, and many tokenized assets are PERMISSIONED. Contract analysis of a token now detects this: an ERC-3643 (T-REX) token gates every transfer through an on-chain identity registry + compliance rules (`onChain.permissioned = {standard:"ERC-3643", identityRegistry, compliance, paused}`); an ERC-1400 controllable security token can be force-moved by a controller (`{standard:"ERC-1400", controllable:true}`). For a swap intent whose tokenIn/tokenOut is ERC-3643, the engine pre-checks the acting wallet's eligibility (`isVerified` on the identity registry + `isFrozen` on the token) and escalates when the leg would REVERT: `components.tokenInEligibility`/`tokenOutEligibility` = `{verified, frozen}`. Lead with this when present: a permissioned token needs eligibility, not just balance, and a naive transfer to an unverified address simply reverts. It is disclosure, never a compliance guarantee.
...
## §F: Fallback to read-only Foundry `cast` (only if Node is unavailable)
